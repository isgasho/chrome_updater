...
func chromePlusScreen(win fyne.Window, data *SettingsData) fyne.CanvasObject {
	...
	checkBtn := widget.NewButtonWithIcon(LoadString("CheckBtnLabel"), theme.SearchIcon(), func() {
		githubReleaseMap, versionList = getChromePlusInfo()
...
func getChromePlusInfo() (map[string]GithubRelease, []string) {
	response, err := http.Get("https://raw.githubusercontent.com/libsgh/ghapi-json-generator/output/v2/repos/Bush2021/chrome_plus/releases%3Fper_page%3D10/data.json")
	if err != nil {
		log.Fatal(err)
	}
